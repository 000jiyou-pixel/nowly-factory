...
# 1. 환경변수 로드 (B봇은 2번 열쇠를 씁니다!)
SUPABASE_URL = os.environ.get("SUPABASE_URL")
SUPABASE_KEY = os.environ.get("SUPABASE_KEY")
GEMINI_API_KEY = os.environ.get("GEMINI_API_KEY_2") # 🚨 수정 완료
...
# ─── 메인 프로세스 ───
def process_trends():
    print(f"\n🚀 [{datetime.datetime.now()}] [B봇] 실시간 크롤링 (6~10위) 시작!", flush=True)
    
    # 🚨 수정: 전체 10개 중 [5번째부터 10번째 끝까지] 뒤쪽 5개만 자릅니다!
    current_trends = create_nowly_ranking()[5:10] 
    ...
    if needs_ai_keywords:
        print(f"\n⚡ [B봇] AI 팩트체크 요약 중... ({len(needs_ai_keywords)}개)", flush=True)
